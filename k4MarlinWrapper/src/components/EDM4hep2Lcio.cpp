...
StatusCode EDM4hep2LcioTool::initialize() { return GaudiTool::initialize(); }
  ...
  CollectionsPairVectors& collection_pairs)
{
  ...
  if (type == "Track") {
    ...
    convertSimTrackerHits(
      collection_pairs.simtrackerhits, collection_pairs.mcparticles, e4h_coll_name, lcio_coll_name, lcio_event);
  }
    ...
    convertSimCalorimeterHits(
      collection_pairs.simcalohits, collection_pairs.mcparticles, e4h_coll_name, lcio_coll_name, lcio_event);
  }
  else if (type == "TPCHit") {
    ...
    convertVertices(
      collection_pairs.vertices, collection_pairs.recoparticles, e4h_coll_name, lcio_coll_name, lcio_event);
  }
  else if (type == "MCParticle") {
...
StatusCode EDM4hep2LcioTool::convertCollections(lcio::LCEventImpl* lcio_event)
{
  ...
  for (int i = 0; i < m_edm2lcio_params.size(); i = i + 3) {
    if (!collectionExist(m_edm2lcio_params[i + 2], lcio_event)) {
      convertAdd(
        m_edm2lcio_params[i], m_edm2lcio_params[i + 1], m_edm2lcio_params[i + 2], lcio_event, collection_pairs);
    }
    else {
