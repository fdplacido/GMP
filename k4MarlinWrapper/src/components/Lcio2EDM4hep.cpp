...
StatusCode Lcio2EDM4hepTool::initialize()
{
  ...
  for (int i = 0; i < m_lcio2edm_params.size(); i = i + 3) {
    if (m_lcio2edm_params[i] == "ReconstructedParticle") {
      m_dataHandlesMap[m_lcio2edm_params[i + 2]] = new DataHandle<edm4hep::ReconstructedParticleCollection>(
        m_lcio2edm_params[i + 2], Gaudi::DataHandle::Writer, this);
    }
...
StatusCode Lcio2EDM4hepTool::convertCollections(lcio::LCEventImpl* the_event)
{
  ...
  convertPut<edm4hep::EventHeaderCollection>("EventHeader", "EventHeader", lcio_converter, id_table);
